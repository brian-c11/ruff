...
1. Convert those into `DefinitionMatch` values, which keep the original `ResolvedDefinition` plus cached metadata needed by references.
1. Resolve each candidate occurrence in the same way.
1. Treat the occurrence as a match when the two `ResolvedDefinition` sets intersect.
1. Convert to LSP-facing `ReferenceTarget`s only after a semantic match has been found.
...
1. Check cross-file parameter keyword references.
...
1. Ignore unrelated dirty files.
